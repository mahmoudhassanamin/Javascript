...
var detailedUsers = [
  ...
  var formattedUsers =transform(detailedUsers);


  function reducerFunc(p,curr,index,arr) {
    if (curr.age < 40){
        p[0]+=curr.age;
        p[1]++;
      }
      if(index === arr.length-1){
        return Math.round(p[0]/p[1]);
      }
      else{
        return p;
      }
  ...
  // var average = formattedUsers.reduce(reducerFunc,[0,0]);